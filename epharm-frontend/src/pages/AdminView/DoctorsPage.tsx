...
import { getAllDoctors, verifyDoctorViaAPI } from "@/api/doctor";
...
const DoctorsPage = () => {
  ...
  const [doctors, setDoctors] = useState([
    ...
    },
    {
      ...
      patients: 92,
      email: "dr.lee@epharm.com",
      status: "active",
      licenseNumber: "MED678901",
      doctor: {
        is_verified: true,
      },
    },
    ...
    },
    {
      ...
      patients: 67,
      email: "dr.clark@epharm.com",
      status: "active",
      licenseNumber: "MED890123",
      doctor: {
        is_verified: false,
      },
    },
  ...
  const verifyDoctor = async (id: string) => {
    // console.log(id);
    const response = await verifyDoctorViaAPI(id);

    toast({
      title: "Doctor Verified Successfully!",
      description: `Doctor with ID ${id} has been verified successfully.`,
    });
                ...
                <TableBody>
                  {doctors.map((doctor) => (
                      <TableRow key={doctor.id}>
                        ...
                        <TableCell>
                          {doctor?.doctor?.license_number || doctor.licenseNumber}
                        </TableCell>
                        <TableCell>{doctor.patients}</TableCell>
                        <TableCell>
                          {getStatusBadge(
                              doctor?.doctor?.is_verified ? "active" : "verification"
                          )}
                        </TableCell>
                        {
                            !doctor?.doctor?.is_verified && (
                                <TableCell className="text-right">
                                  <div className="flex justify-end gap-2">
                                    <Button size="sm" color="green" variant="outline" onClick={() => verifyDoctor(doctor?.doctor?.id)}>
                                      Verify
                                    </Button>
                                  </div>
                                </TableCell>
